...
namespace Swift.Modules.Services.Hotkeys
{
    ...
    public class SwiftHotkeyManager : IHotkeyService, IInitializationAware, IShutdownAware, IPluginServiceUser
    {
        ...
        /// </returns>
        public IDisposable RegisterHotkey(string name, Key key, ModifierKeys modifiers, HotkeyMode mode, HotkeyVisibility visibility, Action<Extensibility.Input.HotkeyEventArgs> callback)
        {
            ...
            {
                token.Binding = new InputBinding(new SimpleCallbackCommand(HandleHotkey, new Swift.Extensibility.Input.HotkeyEventArgs(name)), new KeyGesture(key, modifiers));
        ...
        private void HandleHotkey(Swift.Extensibility.Input.HotkeyEventArgs args)
        {
            var token = _hotkeys[args.Name];
            if (token.Callback != null)
                token.Callback(new Swift.Extensibility.Input.HotkeyEventArgs(args.Name));
            if (token.Visibility == HotkeyVisibility.Public)
            {
                if (OnHotkey != null)
                    OnHotkey(new Extensibility.Input.HotkeyEventArgs(args.Name));
            }
        }

        private class SimpleCallbackCommand : ICommand
        {
            private Action<Swift.Extensibility.Input.HotkeyEventArgs> Callback { get; set; }
            private Extensibility.Input.HotkeyEventArgs Args { get; set; }

            public SimpleCallbackCommand(Action<Swift.Extensibility.Input.HotkeyEventArgs> callback, Extensibility.Input.HotkeyEventArgs args)
            {
                Callback = callback;
                Args = args;
            }
    ...
    public class HotkeyToken : IDisposable
    {
        ...
        /// The callback.
        /// </value>
        public Action<Extensibility.Input.HotkeyEventArgs> Callback { get; private set; }
        ...
        /// <param name="modifiers">The modifiers.</param>
        /// <param name="visibility">The visibility.</param>
        public HotkeyToken(string name, Key key, ModifierKeys modifiers, HotkeyVisibility visibility, Action<Extensibility.Input.HotkeyEventArgs> callback)
        {
            Name = name;
            Key = key;
            Modifiers = modifiers;
